...
function SortableScene({scene,index,total,sceneImg,uploadedImgs,onUpdate,onRemove,onRegen,onPreview,onSelectImg}){
  ...
  const style={transform:CSS.Transform.toString(transform),transition:dndT,opacity:isDragging?.45:1,padding:10,borderRadius:12,background:T.sf,border:`1px solid ${isDragging?T.ac:T.bdr}`,marginBottom:8,cursor:"pointer"}
...
Return ONLY valid JSON:
{
  ...
  const recordVideo=async()=>{
    ...
    try{
      ...
      const bg=ac.createGain();bg.gain.value=(audioUrl||voiceUrl)?.1:.22;bs.connect(bg);bg.connect(dest);bs.start()
    }catch{}
            ...
            <div style={{marginBottom:14}}>
              ...
              {images.length===0?(
                ...
                <div>
                  ...
                  <button onClick={analyzeWithVision} disabled={visionLoading} style={{width:"100%",marginTop:8,padding:"11px",borderRadius:10,background:visionLoading?T.el:`linear-gradient(135deg,${T.ac}cc,#e879f9cc)`,color:"#fff",fontSize:13,fontWeight:700,border:`1px solid ${T.ac}40`,opacity:visionLoading?.6:1,display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>
              ...
              <details style={{marginTop:10}}>
                ...
                <div style={{marginTop:8}}>
                  ...
                  <button onClick={parseHTML} disabled={loading||!htmlSrc.trim()} style={{width:"100%",marginTop:6,padding:"12px",borderRadius:10,background:loading||!htmlSrc.trim()?T.el:`linear-gradient(135deg,${T.or}cc,${T.pk}cc)`,color:"#fff",fontSize:13,fontWeight:700,border:"none",opacity:loading||!htmlSrc.trim()?.5:1}}>
          ...
          <div className="fu">
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(340px,1fr))",gap:16}}>
              ...
              <div>
                ...
                {/* Render + Download */}
                <button onClick={recordVideo} disabled={isRecording||!scenes.length} style={{width:"100%",marginTop:8,padding:"15px",borderRadius:12,background:isRecording?T.rd:`linear-gradient(135deg,${T.ac},${T.pk})`,color:"#fff",fontSize:14,fontWeight:700,opacity:isRecording?.65:1,boxShadow:isRecording?"none":`0 6px 20px ${T.ac}30`}}>
                ...
                {videoBlob&&(
                  <div style={{marginTop:8,display:"flex",flexDirection:"column",gap:6}}>
                    ...
                    {!mp4Blob&&<button onClick={convertToMP4} disabled={converting} style={{width:"100%",padding:"12px",borderRadius:10,border:`1px solid ${T.ac}40`,background:T.acS,color:T.acT,fontSize:13,fontWeight:600,opacity:converting?.5:1}}>{converting?"Đang chuyển đổi...":"🔄 Chuyển sang MP4"}</button>}
              ...
              <div>
                ...
                {replicateKey&&(
                  <div style={{marginBottom:12}}>
                    <button onClick={runProPipeline} disabled={proRendering} style={{width:"100%",padding:"12px",borderRadius:10,background:proRendering?T.el:`linear-gradient(135deg,${T.gn},#059669)`,color:"#fff",fontSize:13,fontWeight:700,border:"none",opacity:proRendering?.6:1,boxShadow:proRendering?"none":`0 4px 16px ${T.gn}30`}}>
